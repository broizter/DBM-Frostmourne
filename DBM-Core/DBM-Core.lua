...
DBM = {
	Revision = parseCurseDate("20260109150122"),
	DisplayVersion = "11.7.0", -- the string that is shown as version
	ReleaseRevision = releaseDate(2026, 01, 09) -- the date of the latest stable version that is available, optionally pass hours, minutes, and seconds for multiple releases in one day
}
...
do
	...
	do
		...
		syncHandlers["DBMv4-L"] = function(sender, version, locale, faction, encounterId, _, lootSourceName, lootSourceGUID, itemID, itemLink, quantity, slot, texture, finalItem) -- version, locale, encounterId, encounterName, lootSourceName, lootSourceGUID, itemID, itemLink, tostring(quantity), tostring(slot), texture, finalItem
			...
			if faction ~= UnitFactionGroup("player") then return end -- don't accept data from other factions since it causes duplicate items if an item has a version for each faction
...
--					end
		...
		if isBarEnabled then
			...
			bar = DBT:CreateBar(timer, id, self.icon, nil, nil, nil, nil, colorId, nil, self.keep, self.fade, countVoice, countVoiceMax, self.simpType == "cd" or self.simpType == "cdnp")
			if not bar then
		...
		if self.simpType and (self.simpType == "cdnp" or self.simpType == "castnp") then--Only send nampelate callback
			fireEvent("DBM_NameplateBegin", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, (self.simpType == "cdnp" and DBM.Options.AlwaysKeepNPs) and true or self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil, isBarEnabled)
			if isBarEnabled then
				fireEvent("DBM_NameplateStart", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, (self.simpType == "cdnp" and DBM.Options.AlwaysKeepNPs) and true or self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil)
			end
		else--Send both callbacks
			fireEvent("DBM_TimerBegin", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil, isBarEnabled)
			if isBarEnabled then
				fireEvent("DBM_TimerStart", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil, isBarEnabled)
			end
			if guid then--But nameplate is only sent if actual GUID
				fireEvent("DBM_NameplateBegin", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil, isBarEnabled)
				if isBarEnabled then
					fireEvent("DBM_NameplateStart", id, msg, timer, self.icon, self.simpType, self.waSpecialKey or self.spellId, colorId, self.mod.id, self.keep, self.fade, self.name, guid, timerCount, self.isPriority, self.type, nil, nil)
				end
			end
		end
